...
class MerossService:
    ...
    def _extract_token_payload(self, http: Any) -> Dict[str, Any]:
        """
        將 meross-iot client 裏面的 token/credentials 轉成 JSON-serializable dict
        """
        # 新版本常見：cloud_credentials 是 MerossCloudCreds object（不可直接 json.dumps）
    ...
    def _json_safe(self, obj: Any) -> Any:
        """
        把 object 變成可 JSON 的資料：
        - dict/list/str/int/bool/None 直接回傳
        - 有 to_dict() 用 to_dict
        - 有 __dict__ 轉 dict（並遞迴處理）
        - 其他 fallback 轉成 string
        """
